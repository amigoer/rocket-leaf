...
- 共享词汇里没有对应条目的能力，应该是一个新端口，而不是 `Attributes` 里的一个角落。
  这个 map 的定义是「某个驱动的 Go 侧与它自己的前端模块之间的约定」，所以放进去的应该是
  规范对象的变体——一个权限、一个持久化标志、一个副本数。Kinesis 的 shard 不是分区的变体：
  它有身份、有哈希键区间、有父分片，把这些塞进字符串 map 会让除了某一个面板之外的任何东西
  都读不懂它。Redis Stream 用四个端口开了这个先例；在伸手去拿那个 map 之前，先问清楚这个
  概念属于哪一种。

- 需要别人先装好客户端库的驱动，等于没人能构建的驱动。伸手去拿厂商 SDK 之前，先看清它链接
  的是什么：`github.com/ibm-messaging/mq-golang` 是基于 IBM 原生 MQ 库的 cgo 绑定，引入它
  会把可再分发客户端塞进本仓库每一次 `go build` 和每一个 CI 任务的关键路径上。有三个家族改用
  厂商自带的 HTTP 管理面接入——ActiveMQ 走 Jolokia，IBM MQ 走 mqweb 服务器的 REST 接口，
  Solace 走 SEMP v2——它们都只用标准库就够了。管理面同时也是数据面的情况，值得先去找一找。

- 名字读起来正是你想要的那个数字，先量一遍再相信它。Solace 的每个队列都上报
  `spooledMsgCount`，它读起来完全像当前深度，实际上是历史累计统计：`clearStats` 会把一个满
  队列的它清零，而被消费空的队列仍停在历史峰值。当前深度要取队列消息集合上的 `meta.count`。
  同一个对象还把存储占用按字节上报，紧挨着的配额却按 MB 上报。这两件事文档里都没有当作陷阱写
  出来，都是往队列里放进已知条数的消息、再把每个字段读回来才发现的——新驱动的第一个小时就该
  花在这上面。

- 管理面在多个地方分别授权的家族，凭据也要多问一次。IBM MQ 的 mqweb 服务器把管理接口和消息
  接口映射到两个角色，部署方可以把它们放在两个账号上——所以表单会收集一份可选的第二凭据，
  连接建立时会探测第二个接口。没有应答的那一层会被标为降级并给出原因，和 ActiveMQ 的 AMQP
  接入点、MQTT 的 $SYS 树一样。绝对不能发生的是一份凭据悄悄顶替另一份：第二对凭据只填了一半
  会被拒绝，而不是拿第一份补齐。第二对凭据留空时是否回退到第一对，由家族自己决定而不是惯例：
  IBM MQ 会回退，因为两个接口读的是同一个用户库；Solace 不会，因为 SEMP 管理用户和 Message
  VPN 的客户端用户名是两个目录里的两种对象。
