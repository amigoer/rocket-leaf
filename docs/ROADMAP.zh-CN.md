...
| 13 | **Amazon SQS** | 队列、消息与发送控制台，连接携带的是 region 与凭证而不是地址 |
| 14 | **Google Cloud Pub/Sub** | 订阅是独立的对象，而不是读取者的位置；其 backlog 映射为 lag |
| 15 | **Azure Service Bus** | peek 不具破坏性，因此这是云托管档里唯一不需要给消息页挂 caveat 的家族；订阅规则进入路由页 |
